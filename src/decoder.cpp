    ...
    void PublisherThreadLoop() {
        while (!stop_publisher_thread_) {
            ...
            if (!frame_to_publish.empty() && dual_publisher_ && front_publisher_ && back_publisher_) {
                ...
                cv::Mat front_img_rotated;
                cv::Mat back_img_rotated;
                cv::rotate(front_img, front_img_rotated, cv::ROTATE_90_CLOCKWISE);
                cv::rotate(back_img, back_img_rotated, cv::ROTATE_90_COUNTERCLOCKWISE);

                auto front_img_msg = std::make_unique<sensor_msgs::msg::Image>();             
                std_msgs::msg::Header front_header;
                front_header.stamp = this->get_clock()->now();
                front_header.frame_id = "front_camera_frame";
                cv_bridge::CvImage cv_image_front(front_header, sensor_msgs::image_encodings::BGR8, front_img_rotated);
                ...
                cv_bridge::CvImage cv_image_back(back_header, sensor_msgs::image_encodings::BGR8, back_img_rotated);
